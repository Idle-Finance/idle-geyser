// Referenced from https://github.com/Idle-Labs/idle-batched-mint/blob/master/lib/HardwareSigner.js
...
class HardwareSigner extends ethers.Signer {
  ...
  async getAddress() {
    const account = await this.retry(`getAddress ${this.i++}`, () => this.eth.getAddress(this.path));
  ...
  async signTransaction(transaction) {
    ...
    const sig = await this.retry(`signTransaction ${this.i++}`, async () => this.eth.signTransaction("m/44'/60'/0'/0/0", unsignedTx));
